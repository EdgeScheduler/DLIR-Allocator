...
def PlotAccumulates(benchs: list, envs: list):
    ...
    for env in envs:
        ...
        for i in range(len(total_labels)):
            plt.plot(total_xs, total_ys_list[i]+[total_ys_list[i][-1]]*(len(total_xs)-len(total_ys_list[i])), linewidth=(1.0 if total_labels[i]=="DLIR" else 0.5),color=mycolor(i),label=total_labels[i], linestyle=("-" if total_labels[i]=="DLIR" else "--"))
